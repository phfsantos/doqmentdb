...
function QueryBuilder(){
    ...
    this.limit = function(l){
     ...
     this.build = function(object) {
         ...
         * @returns {String}
         */
        function udfQuery(value, key, fKey) {
         ...
         * @returns {String}
         */
        function queryArray(object, not, from, operation, previousKey) {
            var subLevel = null;
            ...
            var queryArrayResult = _.keys(object).map(function(key) {
                ...
                if(QUERIES[key]) return udfQuery(object, previousKey, key);
                previousKey = key;
                if(_.isObject(value)) {
                    var fKey = _.first(_.keys(value));
                    // if it's a condition operator || function
                    if(QUERIES[fKey]) {
                        return udfQuery(value, key, fKey);
                    // if it's a conjunctive operator
                        ...
                        subLevel = queryArray(value, not, key, operation, previousKey?previousKey:key);
                        if(subLevel){ 
