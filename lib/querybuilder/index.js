...
function QueryBuilder() {
    ...
    this.clear = function (o) {
        _select = _join = _from = _where = _order = _limit = undefined;
        if (o) {
            o._docs = o._self = undefined;
            delete o._docs;
            delete o._self;
        }
    ...
    this.build = function (object) {
        ...
        var udfArray = [],
            ...
            fromAlias = fromFragments[fromFragments.length - 1],
        ...
        if (_.type(object) == "Document") {
            ...
            //ORDER clause
            BASE_QUERY += _order ? " ORDER BY " + fromAlias + "." + _order : "";
                ...
                if(_.isObject(value)) {
                    ...
                    if(QUERIES[fKey]) {
                        ...
                        if(type === "built-in" && _.isObject(operation)){
                            return filter.format(format, key, operation.func(value[fKey], key));
                        }
        ...
        for(var i = 0; i < keys.length; i++){
            var key = keys[i];
            var replaceFrom = new RegExp("(\\W)(" + from +  "\\." + ")?" + key, "g");
            queryStr = queryStr.replace(replaceFrom, "$1" + from + "." + key);
        }

        // Remove AS ${from} of the query string
        queryStr = queryStr.replace(`AS ${from}.`, "");
