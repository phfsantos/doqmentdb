...
Collection.prototype.select = function(select){
    ...
    return using(select, manager.select.bind(manager));
};
    
Collection.prototype.from = function(from){
    var manager = this.manager;

    return using(from, manager.from.bind(manager));        
};
    
Collection.prototype.join = function(join){
    var manager = this.manager;

    return using(join, manager.join.bind(manager));
};
    
Collection.prototype.where = function(where){
    var manager = this.manager;

    return using(where, manager.where.bind(manager));
};
    
Collection.prototype.order = function(order){
    var manager = this.manager;

    return using(order, manager.order.bind(manager));
};
    
Collection.prototype.limit = function(limit){
    var manager = this.manager;

    return using(limit, manager.limit.bind(manager));
};
