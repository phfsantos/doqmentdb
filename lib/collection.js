 ...
 * @returns {*}
 */
Collection.prototype.sql = function(queryString) {
    return _sql.call(this, queryString, true);
};
...
Collection.prototype.create = function(doc) {
    ...
    function fn() {
        return using(_getCollection.call(self), doc, function (coll, doc) {
            // Add in an createdAt property as a time stamp
            var ts = new Date();
            if (typeof doc == "object"){
              doc.createdAt = ts.getTime();
            }
...
Collection.prototype.select = function(select){
    var manager = this.manager;
    return manager.select(select);
};
    
Collection.prototype.from = function(from){
    var manager = this.manager;
...
Collection.prototype.join = function(join){
    var manager = this.manager;
    return manager.join(join);    
};
    
Collection.prototype.where = function(where){
    var manager = this.manager;
    return manager.where(where);    
};
    
Collection.prototype.order = function(order){
    var manager = this.manager;
...
/**
 * @description
 * get query sql from manager
 * @param obj
 * @returns {*}
 * @private
 */
function _sql(queryString) {
    let manager = this.manager;
    let coll = _getCollection.call(this);
    return using({_self: true},queryString, manager.sql.bind(manager));
}
...
function _find(object, many) {
    ...
    , q = using(_getCollection.call(this), object, function(coll) {        
        if(object) object._self = true;
        else object = {_self: true};
