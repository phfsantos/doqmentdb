...
var _         = require('./utils')
  ...
  , QueryBuilderClass = require('./querybuilder')
  , using     = Promise.using
...
Manager.prototype.query = function(object, options) {
  var queryString = querybuilder.build(object)
...
Manager.prototype.findOrCreate = function(parent, object) {
  ...
  var findMethod = _.isUndefined(object) ? 'queryDatabases' : ['query', _.isDataBase(parent) ? 'Collections' : 'Documents'].join('');
  var findArgs = _.isUndefined(object) ? [] : [parent._self];
  findArgs.push(querybuilder.build(object || parent));
  return new Promise(function(resolve, reject) {
...
Manager.prototype.select = function(select){
    return using(select, querybuilder.select.bind(querybuilder));
};
    
Manager.prototype.from = function(from){
    return using(from, querybuilder.from.bind(querybuilder));        
};
    
Manager.prototype.join = function(join){
    return using(join, querybuilder.join.bind(querybuilder));
};
    
Manager.prototype.where = function(where){
    return using(where, querybuilder.where.bind(querybuilder));
};
    
Manager.prototype.order = function(order){
    return using(order, querybuilder.order.bind(querybuilder));
};
    
Manager.prototype.limit = function(limit){
    return using(limit, querybuilder.limit.bind(querybuilder));
};

Manager.prototype.clear = function(){
    return querybuilder.clear();
};
  ...
  return function(parent, params) {
    var client = this.client
      ...
      , method = isQuery
        ? one   // query...
        : all   // read...
      , args = [parent._self].concat(isQuery ? [querybuilder.build(params)] : []);
    return new Promise(function(resolve, reject) {
