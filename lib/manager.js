 ...
 */
Manager.prototype.find = function (parent, select, from, join, where, order, many, udf) {
  var client = this.client
    , type = _.isDataBase(parent) ? 'Collections' : 'Documents'
    , method = 'query'
    , queryString = this.query(select, from, join, where || {}, order, udf ? { udf: udf, parent: parent } : {});
 ...
 * @returns {*}
 */
Manager.prototype.query = function(select, from, join, where, order, options) {
  var queryString = query(select, from, join, where, order)
  ...
  return function(parent, params) {
    var client = this.client
      ...
      , method = isQuery
        ? one   // query...
        : all   // read...
      , args = [parent._self].concat(isQuery ? [query(null, null, null, params)] : []);
    return new Promise(function(resolve, reject) {
