 ...
 * @expose
 */
module.exports = function(select, from, join, where, order) {
  /**
   * @description
  ...
  /**
   * @description Parse where clause from an object like in mongodb
   * query builder
   * @param {Object} object
   * @param {Boolean=} not
   * @returns {String}
   */
  function query(select, from, join, where, order) {
      var query = "";
      //SELECT CLAUSE
      query += "SELECT" + select?" * ":select + " ";
      //FROM CLAUSE
      query += "FROM" + !from?" root ":from + " ";
      //JOIN CLAUSE
      query += join?  "JOIN" + join + " ":"";
      //WHERE CLAUSE
      query +=  where?"WHERE" + where + " ":"";
      //ORDER CLAUSE
      query +=  where?"ORDER BY" + order + " ":"";
      return query;
  }  
   ...
   * @returns {String}
   */
  function parseWhere(object, not) {
   ...
   */
  var queryStr = query(select, from, join, (_.isString(where) ? where  : parseWhere(where)), order);  // concat build query string
